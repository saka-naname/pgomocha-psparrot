import React, { memo } from "react";

import { useState } from "react";

const Parrot = memo((props) => {
    const { fillBody } = props;
    const fillNose = "#7b8c68";

    return (
        <>
            <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0" y="0" viewBox="0 0 480 480" enableBackground="new 0 0 480 480" xmlSpace="preserve" width={160} height={160}>
                <style>.eye`&lbrace`fill:#000`&rbrace`</style>
                {/* <path d="M448 330.5c-2.6-2.9-6.4-7.1-11.3-12-27.1-27-52.4-41.3-64.7-48.7-19-11.4-45.3-29.2-76-57.3 1.3-7.3 2.3-15.6 2.7-24.7.6-15.7-.9-29.5-3-40.6-1.3-4.7-4-12-9.7-19.4-7.3-9.5-15.9-14.4-20.7-16.7-.4-2.2-.9-4.4-1.3-6.7-2.2-10.8-4.5-21.5-7-31.9-5.2-9.2-22.3-36.8-57.7-51.4-5.8-2.4-45.6-18.2-88-.7C89.4 29.6 75.6 43.9 70 49.9 58.2 62.4 52 74.2 43.3 90.5c-6.7 12.7-13.8 28.3-20 46.7-1.9 1.1-6 3.7-9 8.8-3.4 5.8-3.4 11.3-3.3 13.5-.1 8.8.5 19.3 2.5 31 1.6 9.1 3.7 17.1 6 24 2.4 5.4 5 9.1 7 11.5 1.3 1.6 2.3 2.6 3.5 4.5 2.3 3.7 3 7.4 3.3 10 .7 13.9.9 29.1.3 45.4-.6 16-1.9 30.9-3.7 44.6H448z" fill={fillBody} stroke="#000" strokeWidth={14} strokeLinejoin="round" strokeMiterlimit={10} />
                <path d="M32.8 250.6c21.9 22.2 42.9 35.2 54.4 39.5 8.9 3.3 18.6 5.4 18.6 5.4 5.6 1.2 10.6 1.9 14.5 2.4 4.2 0 11.9.5 19.5 5 2.5 1.5 4.5 3.1 6.1 4.6" fill="none" stroke="#000" strokeWidth={12} strokeLinecap="round" strokeMiterlimit={10} />
                <ellipse className="eye" cx="111" cy="88.3" rx="10" ry="21"/>
                <ellipse className="eye" cx="234" cy="88.3" rx="10" ry="21"/>
                <path d="M181.7 223.8c-.9-.3-1.7-.6-2.6-.9-2.2-1-4.2-2.3-5.9-3.9-4.3-5.8-8.4-12-11.9-18.3-5-8.8-9.3-18.2-12.7-27.8-1.8-12.2-2.8-24.6-3.2-36.7-.1-4.6-.1-6.2 0-10.1.1-3.2.2-8.1.3-17.3 0-2.5.1-6.7.9-12.1.2-1.4.5-2.9.8-4.3.6-.4 1.3-.8 2-1.1 1.7-.7 3.2-.8 4.3-.9 5.4-.3 11.3-.5 17-.7h29.2c3 2.5 5.6 5.4 7.8 8.6 2 3.1 3.6 6.4 4.8 9.9.5 4.5.8 8.9.9 13.2v4.6c0 1.5-.1 2.9 0 5.9.1 1.9.2 3.5.3 4.8.2 2.5.3 3.9.1 6.2-.2 2-.5 4-1 6-3.3 6.6-6.3 13.3-8.9 20-1.9 5-3.5 8.9-4.9 14.4-.8 3.2-1.2 5.2-1.5 7.1-.3 1.7-.6 3.4-1.2 5.9l-.2.9c-.7 3.1-1.9 8.2-4 12.5-2.7 5.3-6.2 10-10.4 14.1z" fill={fillNose} stroke="#000" strokeWidth={12} strokeLinecap="round" strokeMiterlimit={10} /></svg> */}
                <path d="M448 430.5c-2.6-2.9-6.4-7.1-11.3-12-27.1-27-52.4-41.3-64.7-48.7-19-11.4-45.3-29.2-76-57.3 1.3-7.3 2.3-15.6 2.7-24.7.6-15.7-.9-29.5-3-40.6-1.3-4.7-4-12-9.7-19.4-7.3-9.5-15.9-14.4-20.7-16.7-.4-2.2-.9-4.4-1.3-6.7-2.2-10.8-4.5-21.5-7-31.9-5.2-9.2-22.3-36.8-57.7-51.4-5.8-2.4-45.6-18.2-88-.7C89.4 129.6 75.6 143.9 70 149.9 58.2 162.4 52 174.2 43.3 190.5c-6.7 12.7-13.8 28.3-20 46.7-1.9 1.1-6 3.7-9 8.8-3.4 5.8-3.4 11.3-3.3 13.5-.1 8.8.5 19.3 2.5 31 1.6 9.1 3.7 17.1 6 24 2.4 5.4 5 9.1 7 11.5 1.3 1.6 2.3 2.6 3.5 4.5 2.3 3.7 3 7.4 3.3 10 .7 13.9.9 29.1.3 45.4-.6 16-1.9 30.9-3.7 44.6H448z" fill={fillBody} stroke="#000" strokeWidth={14} strokeLinejoin="round" strokeMiterlimit={10} />
                <path d="M32.8 350.6c21.9 22.2 42.9 35.2 54.4 39.5 8.9 3.3 18.6 5.4 18.6 5.4 5.6 1.2 10.6 1.9 14.5 2.4 4.2 0 11.9.5 19.5 5 2.5 1.5 4.5 3.1 6.1 4.6" fill="none" stroke="#000" strokeWidth={12} strokeLinecap="round" strokeMiterlimit={10} />
                <ellipse className="eye" cx="111" cy="188.3" rx="10" ry="21"/>
                <ellipse className="eye" cx="234" cy="188.3" rx="10" ry="21"/>
                <path d="M181.7 323.8c-.9-.3-1.7-.6-2.6-.9-2.2-1-4.2-2.3-5.9-3.9-4.3-5.8-8.4-12-11.9-18.3-5-8.8-9.3-18.2-12.7-27.8-1.8-12.2-2.8-24.6-3.2-36.7-.1-4.6-.1-6.2 0-10.1.1-3.2.2-8.1.3-17.3 0-2.5.1-6.7.9-12.1.2-1.4.5-2.9.8-4.3.6-.4 1.3-.8 2-1.1 1.7-.7 3.2-.8 4.3-.9 5.4-.3 11.3-.5 17-.7h29.2c3 2.5 5.6 5.4 7.8 8.6 2 3.1 3.6 6.4 4.8 9.9.5 4.5.8 8.9.9 13.2v4.6c0 1.5-.1 2.9 0 5.9.1 1.9.2 3.5.3 4.8.2 2.5.3 3.9.1 6.2-.2 2-.5 4-1 6-3.3 6.6-6.3 13.3-8.9 20-1.9 5-3.5 8.9-4.9 14.4-.8 3.2-1.2 5.2-1.5 7.1-.3 1.7-.6 3.4-1.2 5.9l-.2.9c-.7 3.1-1.9 8.2-4 12.5-2.7 5.3-6.2 10-10.4 14.1z" fill={fillNose} stroke="#000" strokeWidth={12} strokeLinecap="round" strokeMiterlimit={10} /></svg>
        </>
    );
});

export default Parrot;
